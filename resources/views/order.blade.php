...
<section class="section">
    <div class="card">
      ...
      @endforeach
          <div class="card-top border-bottom text-center"><a href="{{ route('basket')}}"><h5><i class="fas fa-backward mr-1"></i>Back to cart</h5></a> <span id="logo">Checkout</span> </div>
              <div class="row upper"></div>
              <div class="row">
                  <div class="col-md-7">
                      <div class="left border">
                          <div class="row"> <span class="header">Buyer parameters</span>
                          ...
                          <form action="{{route('new_order_place')}}" method="post">
                            <span>Name:</span> <input placeholder="{{Auth::check()?Auth::user()->name:'Enter your name'}}" name="name">
                            <span>Phone number:</span> <input class="phone" onClick="setPos();" placeholder="{{Auth::check()?(Auth::user()->phone?Auth::user()->Format_Number_User:'Enter your phone number'):'Enter your phone number'}}" name="phone">
                            <span>E-MAIL:</span> <input placeholder="{{Auth::check()?Auth::user()->email:'Enter your email'}}" name="e-mail">
                            @csrf
                              <div class="row">
                                  <!-- <div class="col-4"><span>Город:</span> <input placeholder="Baku"></div> -->
                                  <div class="col-4"><span>address:</span> <input placeholder="Введите адрес" name="address"></div>
                  ...
                  <div class="col-md-5">
                      <div class="right border">
                          <div class="header">Order summary <span class="prodcount">{{ Session::has('cart') ? Session::get('cart')->totalQty : '0' }}</span></div><hr>
                          @foreach($products as $product)
                          <h6><a href="/{{ $product['code_cat'] }}/{{ $product['url_cat'] }}/{{ $product['prod_url'] }}">{{ $product['name'] }}</a></h6>
                          <div class="row item">
                              <div class="col-4 align-self-center"><a href="/{{ $product['code_cat'] }}/{{ $product['url_cat'] }}/{{ $product['prod_url'] }}"><img class="basketimg mr-1" src="{{(!is_null($product['img']))?($product['img']):('/img/products/no-img.png')}}"></a></div>
                              <div class="col-8">
                          ...
                          <div class="row lower">
                              ...
                              <div class="col text-right">To be specified separately</div>
                          </div>
                          <!-- <div class="row lower">
