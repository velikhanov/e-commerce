     ...
     @foreach($categories as $category)
        <div class="col-lg-2 col-md-4 col-sm-6">
            <a href="/{{ Request::is('categories') ? $category->url : $category->code}}{{ Request::is('categories') ? '' : '/' . $category->url }}" class="btn btn-lg btn-success catbtn" type="button">
              ...
              <img class="catbtnimg" src="{{Storage::disk('public')->exists('categories/'.$category->id.'/'.$category->img) ? Storage::disk('public')->url('categories/'.$category->id.'/'.$category->img) : '/img/categories/kitchen-utensils.png'}}">
            </a>
