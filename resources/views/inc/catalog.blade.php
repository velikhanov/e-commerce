             ...
             <li class='has-sub'><a href="#"><span>Каталог</span></a>
                <ul>
                  @foreach( $catalog as $item )
                   <li class='has-sub'><a href="#"><img class="catalogimg" src="@isset($item->img && $catimg){{$catimg}}@else /img/categories/kitchen-utensils.png @endisset"><span class="cat-text">{{ $item->name }}</span></a>
                      <ul>
                        @foreach( $item->children as $subitem )
                         <li><a href='/{{ $item->url }}/{{ $subitem->url }}'><img class="catalogimg" src="@isset($subitem->img && $catimg){{$catimg}}@else /img/categories/kitchen-utensils.png @endisset"><span class="cat-text">{{ $subitem->name }}</span></a></li>
                        @endforeach
                      </ul>
                   </li>
                   @endforeach
