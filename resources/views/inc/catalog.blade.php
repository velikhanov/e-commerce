                  ...
                  @endif"><span class="cat-text">{{ $item->name }}</span></a>
                   <ul>
                    @foreach( $item->children as $subitem )
                     <li><a href='/{{ $subitem->url }}/{{ $subitem->url }}'><img class="catalogimg" src="@if(!is_null($subitem->img_catalog_url)){{$subitem->img_catalog_url}}@else /img/categories/kitchen-utensils.png @endif"><span class="cat-text">{{ $subitem->name }}</span></a></li>
                    @endforeach
                   </ul>
                  </li>
                 @endforeach
